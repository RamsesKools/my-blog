...
I always add a CI step that reruns the pre-commit hooks in the pipeline itself.
That way every change, including from people who forgot to run `pre-commit install`, gets checked against the same hooks before it can land.

## Teaching AI agents to use it

I make a point of teaching my AI coding agents to use pre-commit too.
